...
with tab4:
    ...
    2. **一般設定區（關鍵治理選項！）**：
       ...
       * **錯誤則停止**：選擇「是」。
    3. **從檔案匯入試題**：將 STEP 3 下載的 `.txt` 檔（例如：`20260814週五_詞彙記憶_Moodle題庫.txt`）拖入虛線上傳框，點擊藍色 **「匯入」** 按鈕。
    """)

    st.markdown("---")

    st.markdown("### ⚙️ 第三階段：Moodle 測驗卷 (Quiz) 活動參數設定 (對照實機欄位)")
    st.markdown("""
    於課程頁面點擊「新增活動或資源」➔ 選擇「測驗 (Quiz)」，完成核心設定：

    1. **版面設計 ➔ 新頁面**：設定為 **「每一題」** (單題刷卡體驗)。
    2. **版面設計 ➔ 導覽方式**：改為 **「順序」** 🛑 (確保依照形義 ➔ 聽力 ➔ 手寫填空階梯式順序解鎖)。
    3. **試題的作答與計分方式（關鍵對照欄位！）**：
